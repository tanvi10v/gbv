...
let birdFrames = ['female_walk1', 'female_walk2']; // The frames of the bird
let bird;
...
let warningSigns = ['red_flag', 'silence_mouth', 'Hand_with_Raised_Fist', 'cracked_heart', 'broken_chain']; // Array to hold the warning signs

function preload() {
  this.load.image('background', 'assets/background.png');
  this.load.image('female_walk1', 'assets/female_walk1.png');
  this.load.image('female_walk2', 'assets/female_walk2.png');
  this.load.image('base', 'assets/base.png');
  this.load.image('piller', 'assets/pipe-red.png');
  this.load.image('startGame', 'assets/start_game_new.png');
  this.load.image('gameOver', 'assets/label_game_over.png');
  this.load.image('resumeButton', 'assets/button_resume.png');

  this.load.image('red_flag', 'assets/warningSigns/red_flag.png');
  this.load.image('silence_mouth', 'assets/warningSigns/silence_mouth.png');
  this.load.image('Hand_with_Raised_Fist', 'assets/warningSigns/Hand_with_Raised_Fist.png');
  this.load.image('cracked_heart', 'assets/warningSigns/cracked_heart.png');
  this.load.image('broken_chain', 'assets/warningSigns/broken_chain.png');
...
function create() {
  ...
  startGameImage.on('pointerdown', () => {
    ...
    // Create a warning sign randomly every 5 seconds
    this.time.addEvent({
      delay: 5000,
      callback: () => {
        if (gameOver) {
        ...
        createWarningSign(); // Call the createWarningSign function
      },
      loop: true
    });


    const createWarningSign = () => {
      // Randomly pick a warning sign from the array
      const randomWarningSign = warningSigns[Math.floor(Math.random() * warningSigns.length)];
      // Randomly pick a position for the warning sign
      const randomX = Phaser.Math.Between(0, game.config.width);
      const randomY = Phaser.Math.Between(0, game.config.height - base.height);
      // Create the warning sign sprite
      let warningSign = this.physics.add.sprite(randomX, randomY, randomWarningSign);
      warningSign.displayHeight = 100; // Set the height of the warning sign
      warningSign.displayWidth = 100; // Set the width of the warning sign
      warningSign.setOrigin(0.5, 0.5); // Set the origin to the center
      warningSign.setVelocityX(-200); // Move the warning sign to the left
      // Set the warning sign to be interactive 
      warningSign.setInteractive(); // Make the warning sign interactive 
    }
  })

  bird = this.physics.add.sprite(game.config.width / 2, game.config.height / 2, 'female_walk1'); // Create the bird sprite
